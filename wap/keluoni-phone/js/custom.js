...
//预约弹窗函数
function tanchuyuyue(obj){
	var whosname = $(obj).parent().find('.name').html();
	$('.whosname').val(whosname);
	var yuyue = layer.open({
		type: 1,
		title: false,
		closeBtn: false,
		fix: true,
...
//设计师弹窗函数
function tanchudesigner(i){
	var designer = layer.open({
		type: 1,
		title: false,
		closeBtn: false,
		fix: true,
		...
		shift:2,
		maxmin: false,
		content: $('.designer_detail_'+i),
		zIndex:9998
	});
	layer.full(designer);
	$('.tc-back').on('click',function(){
		layer.close(designer);
	})
}
//输入框最大数值函数
function MaxScore(e){
...
$(function(){
    ...
    if (window.screen.width > 700) {
        window.onscroll = function() {
            ...
            var scroll_top_start = 400;                 //滚动条触发事件的距离
            //二级菜单浮动
